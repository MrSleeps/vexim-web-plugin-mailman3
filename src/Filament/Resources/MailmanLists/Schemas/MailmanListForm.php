                ...
                Action::make('edit')
                    ...
                    ->action(function (array $data, $record, $livewire) use ($listId) {
                        try {
                            ...
                            // Update the user's display name directly
                            $nameUpdated = $mailman->updateUserDisplayName($record['email'], $data['display_name']);

                            $emailChanged = $data['email'] !== $record['email'];

                            if ($emailChanged) {
                                // If email changed, unsubscribe old and subscribe new
                                ...
                                if ($nameUpdated) {
                                    ...
                                    // Fallback: unsubscribe and resubscribe with new name
                                    $mailman->unsubscribe($listId, $record['email']);
                                    $mailman->subscribe($listId, $data['display_name'], $data['email']);
